...
/**
 * Class for handing the interface between XIV Recorder and OBS.
 *
 * This works by constantly recording a "buffer" whenever FFXIV is open. If an
 * interesting event is spotted in the IINACT log (e.g. a zone change to a
 * CC arena), the buffer becomes a real recording.
 *
 * This ensures we catch the start of activities, the fundamental problem
 * here being that the log doesn't write in real time, and we might
 * actually see the event after it occurred in game.
 */
export default class Recorder extends EventEmitter {
   ...
   */
  private resolution: string = this.cfg.get<string>(
    'obsOutputResolution',
  );
  ...
  private setupListeners() {
    ...
    ipcMain.handle('audioSettingsOpen', () => {
      ...
      if (Poller.getInstance().isFFXIVRunning()) {
        console.info('[Manager] Wont touch audio sources as FFXIV is running');
        return;
      }
  ...
  public async configureBase(config: BaseConfig, startup: boolean) {
    ...
    this.resolution = obsOutputResolution;
    const [widthStr, heightStr] = this.resolution.split('x');
  ...
  public configureVideoSources(config: ObsVideoConfig) {
    ...
    const ffxivRunning = Poller.getInstance().isFFXIVRunning();

    if (ffxivRunning && obsCaptureMode !== 'monitor_capture') {
   ...
   * so it can be called externally when FFXIV is opened.
   */
  public configureAudioSources(config: ObsAudioConfig) {
    ...
    config.audioSources.forEach((src) => {
      ...
      if (src.type === AudioSourceType.PROCESS && src.device) {
        ...
        const available = properties.find((prop: any) => prop.name === 'device_id');
        assert(available && available.type === 'list'); // To help the compiler out.

        // Try to match by device ID.
        let match = available.items.find((d: any) => d.value === src.device);
        ...
        if (!match) {
          ...
          match = available.items.find((d: any) => d.name === src.friendly);
   ...
   * so it can be called externally when FFXIV is closed.
   */
  ...
  private configureMonitorCaptureSource(config: ObsVideoConfig) {
    ...
    const monitors = properties.find((p: any) => p.name === 'monitor_id');
    ...
    const opts = monitors.items.filter((item: any) => item.value !== 'DUMMY');
  ...
  /**
   * Check if the name of the window matches the FFXIV window name.
   */
  private static windowMatch(item: { name: string; value: string | number }) {
    return item.name.startsWith('[ffxiv_dx11.exe]: ');
  }

  /**
   * Attach the current game_capture or window_capture source to the FFXIV client.
   */
  public attachCaptureSource() {
    ...
    const properties = noobs.GetSourceProperties(this.captureSource);
    const windows = properties.find((item: any) => item.name === 'window');
    ...
    console.warn(
      '[Recorder] Failed to find FFXIV window after',
      this.findWindowAttempts,
      'attempts. Giving up.',
    );
  }
  ...
  public setSourcePosition(
    ...
    // We only allow one scale factor to retain the aspect ratio of
    ...
    const item = src.startsWith('XIVRecorder Chat Overlay')
      ? SceneItem.OVERLAY
      : SceneItem.GAME;
