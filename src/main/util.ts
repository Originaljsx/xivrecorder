...
// FFXIV constants (no WoW install search paths needed)
import { defaultIINACTLogPath } from './constants';
...
const setupApplicationLogging = () => {
  ...
  const logRelativePath = `logs/XIVRecorder-${date}.log`;
 ...
 */
// Korean category conversion removed — not needed for FFXIV.
...
// getWowFlavour removed — not needed for FFXIV.
 ...
 * path that contains other files which XIV Recorder may go on to delete.
 * More context: https://github.com/aza547/wow-recorder/issues/400.
 */
const takeOwnershipStorageDir = async (dir: string) => {
  const helptext =
    'If you are setting up XIV Recorder for the first time, this folder should be empty.';

  const content =
    'This folder is managed by XIV Recorder, files in it may be automatically created, modified or deleted.';

  const files = await fs.promises.readdir(dir);

  // Check for any files that don't match the extensions XIV
  ...
  // recording MP4s to the same directory as they configured XIV Recorder
  // to use.
 ...
 * storage path that contains other files which XIV Recorder may go on to delete.
 * More context: https://github.com/aza547/wow-recorder/issues/400.
 */
...
const runFirstTimeSetupActionsNoObs = () => {
  ...
  // Auto-detect IINACT log directory.
  if (!cfg.get<string>('iinactLogPath')) {
    const iinactPath = defaultIINACTLogPath();

    if (existsSync(iinactPath)) {
      console.info('[Util] Found IINACT log directory at', iinactPath);
      cfg.set('iinactLogPath', iinactPath);
    } else {
      console.warn('[Util] IINACT log directory not found at default path');
    }
  }
  ...
  if (!cfg.get<string>('storagePath')) {
    ...
    const initialStorageDir = path.join(baseVideoPath, 'XIV Recorder');
