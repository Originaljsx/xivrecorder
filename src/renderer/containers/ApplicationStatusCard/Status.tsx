...
const Status = ({
  ...
  const getConfiguredCategories = () => {
    ...
    if (config.recordCrystallineConflict) {
      categories.push('Crystalline Conflict');
    }
  ...
  const RecStatusTitle: Record<RecStatus, string> = {
    ...
    [RecStatus.WaitingForFFXIV]: getLocalePhrase(
      language,
  ...
  const RecStatusVariants: Record<RecStatus, StatusLightProps['variant']> = {
    ...
    [RecStatus.WaitingForFFXIV]: 'waiting',
        ...
        <ul className="text-xs text-popover-foreground/60 list-disc pl-4">
          {config.iinactLogPath && (
            <li>
              ...
              <code>{config.iinactLogPath}</code>
            </li>
          )}
        </ul>
        <Separator className="my-1" />
