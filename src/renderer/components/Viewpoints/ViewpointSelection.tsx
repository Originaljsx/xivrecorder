...
export default function ViewpointSelection(props: IProps) {
  ...
  const mapCombatants = (combatant: RawCombatant) => {
    ...
    if (matches.length > 0) {
      // We only bother to get a job if we have a match. That way the
      // combatants we have a viewpoint for will be colored, else they will
      // be gray.
      const v = matches[0];
      jobId = v.player?._jobId;
    }
    ...
    const classColor = jobId !== undefined ? getJobRoleColor(jobId) : 'gray';
    ...
    return (
      ...
      >
        ...
        >
          {jobAbbr && (
            <span
              className="absolute top-[2px] left-[2px] text-black font-bold text-[8px]"
            >
              {jobAbbr}
            </span>
          )}
  ...
  if (isCC) {
    // For Crystalline Conflict we split the teams.
