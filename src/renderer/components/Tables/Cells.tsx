...
export const populateViewpointCell = (
  ...
  if (!player || !player._name) {
    // We don't have enough to render a name so
    // just return the viewpoint count.
    return <div>{count}</div>;
  }

  const playerName = getPlayerName(first);
  const playerJobId = getPlayerJobId(first);
  const jobColor = getJobRoleColor(playerJobId);
  const jobAbbr = getJobAbbreviation(playerJobId);
  ...
  const renderJobAndName = () => {
    return (
      <>
        {jobAbbr && (
          <span
            className="font-sans font-bold text-xs mx-1"
